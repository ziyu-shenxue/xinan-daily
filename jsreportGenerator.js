...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // 绑定表单输入事件，防抖后同步预览（解决问题13：实时更新无需手动刷新）
  const debouncedPreview = debounce(generateLivePreview, 300);
  document.querySelectorAll('.core-component').forEach(component => {
    // 监听输入事件（输入框、滑块等实时变化）
    component.addEventListener('input', debouncedPreview);
    // 监听变化事件（下拉框、复选框等选择变化）
    component.addEventListener('change', debouncedPreview);
    // 监听文本域输入（补充文本域实时更新）
  ...
  // 暴露提取亮点函数（双重保障，关联jsFormInteraction.js）
  if (typeof extractHighlights === 'function' && !window.extractHighlights) {
    window.extractHighlights = extractHighlights;
  }
  
  console.log('✅ 报告生成与导出组件初始化完成：支持实时防抖预览、PDF/PNG/JSON导出、草稿保存/读取');
});

// 补充：确保提取亮点函数全局可访问（防止核心交互JS加载顺序问题）
if (typeof extractHighlights === 'function' && !window.extractHighlights) {
  window.extractHighlights = extractHighlights;
}